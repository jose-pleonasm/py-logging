...
Configurator.jsonConfig = function(config) {
	...
	for (var identifier in config.loggers) {
		...
		var loggerParams = config.loggers[identifier];
		var classParam = identifier;
		...
		_descriptors[identifier] = { params: loggerParams };
		instancies.loggers[identifier] = getLogger(classPath);
	}
	Configurator._setComponents(
		_descriptors,
		instancies.loggers,
