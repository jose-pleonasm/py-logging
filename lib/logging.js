...
Configurator._getSectionParams = function(section, defaultClass) {
	...
	for (var identifier in section) {
		...
		var settings = Object.assign({}, descriptor);
		var classPath = Configurator._createClassPath(klass);
		var context = Configurator._isInternal(klass) ? module.exports : global;
...
Configurator._setComponents = function(descriptors, subjects, components,
	...
	for (var identifier in descriptors) {
		...
		var componentIdentifiers = descriptor.settings[subjectSettingsKey];
...
Configurator._isInternal = function(klass) {
	return klass.indexOf(MODULE_IDENTIFIER + '.') === 0;
};

Configurator._createClassPath = function(klass) {
	return Configurator._isInternal(klass)
